...
def read_openpose_json(openpose_output_dir, smooth=True):
    # openpose output format: [x1,y1,c1, x2,y2,c2,...]
    ...
    j_files = [_file for _file in json_files if len(json.load(
        open(os.path.join(openpose_output_dir, _file)))["people"]) != 0]

    f = lambda a: "00" if a < 10 else("0" if a < 100 else "")
    json_files = [f(j+1) + str(j+1) + "_keypoints.json" for j, _files in enumerate(j_files)]

    #print(j_files)
    #print(json_files)
    
    ### extract x,y and ignore confidence score
    for nb, file_name in enumerate(json_files):
        logger.debug("reading {0}".format(file_name))
        _file = os.path.join(openpose_output_dir, j_files[nb])
    ...
    for frame, xy in cache.items():
        ...
        for x in range(0,_len,2):
            ...
            if frame in head_frame_block:
                ...
                # median value calc: find neighbor frames joint value and sorted them, 
...
def main(_):
    ...
    with tf.Session(config = tf.ConfigProto(
        ...
        input_dir = os.path.join(root_path, pose_input)
        
        for sub in os.listdir(input_dir):
            sub_dir = os.path.join(input_dir, sub)
            print("\nworking on subject: ", sub)

            for angle in os.listdir(sub_dir):
                print("\nworking on angle: ", angle)
                angle_dir = os.path.join(sub_dir, angle)

                for gait_seq  in os.listdir(angle_dir):
                    print("gait sequence: ", gait_seq)
                    seq_dir = os.path.join(angle_dir, gait_seq)

                    smoothed = read_openpose_json(seq_dir)
                    ...
                    # saving files
                    file_3d = gait_seq + ".json"
                    file_2d =  "2d_" + gait_seq + ".json"

                    save_path = os.path.join(root_path, 'output', sub, angle)
                    os.makedirs(save_path, exist_ok= True)

                    _out_file = os.path.join(save_path, file_3d)
                    twod_out_file = os.path.join(save_path, file_2d)
...
if __name__ == "__main__":
    pose_input = FLAGS.pose_estimation_json
